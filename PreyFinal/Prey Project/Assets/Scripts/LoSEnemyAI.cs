...
public class LoSEnemyAI : MonoBehaviour
{
    ...
    void Chase()
    {
        ...
        rayDirection = (player.transform.position - transform.position).normalized;
            ...
            if (timer3 >= maxTimerValueOPAi)
            {
             ...
             Debug.DrawLine(transform.position + Vector3.up * height, player.transform.position, Color.red);
    ...
    void Investigate()
    {
        ...
        if (Physics.Raycast(transform.position + Vector3.up * height, transform.forward, out hit2, range_radius))
        {
        ...
        if (Physics.Raycast(transform.position + Vector3.up * height, (transform.forward + transform.right).normalized, out hit2, range_radius))
        {
            if (hit2.collider.gameObject.tag == "Player")
            {
                playerFound = true;
